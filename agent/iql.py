import sys
sys.path.append("/home/venky/Desktop/RL-VLM-F")
import copy
import os
import random
import uuid
...
class TrainConfig:
    ...
    data_set_path: Optional[str] = "/home/venky/Desktop/RL-VLM-F/test_dummy/cartpole/data_replay_corrected_pred.pkl"
    vlm_reward: Optional[bool] = True
    seed: int = 0  # Sets Gym, PyTorch and Numpy seeds
    eval_iter :int = 25 #Number of evaluations when running eval method - default 10
    eval_freq: int = int(50)  # How often (time steps) we evaluate -default 5000
    n_episodes: int = 10  # How many episodes run during evaluation
    max_timesteps: int = int(1e6)  # Max time steps to run environment - defualt int (1e6)
    checkpoints_path: Optional[str] = "/home/venky/Desktop/RL-VLM-F/offline_rl_exp/cartpole"  # Save path
    ...
    project: str = "RISS"
    group: str = "IQL"
    name: str = "IQL-trial-corrected-eval-50"
    if vlm_reward:
        name = name + "-vlm_reward"
    
    def __post_init__(self):
        self.name = f"{self.name}-{self.env}-{str(uuid.uuid4())[:8]}"
        if self.checkpoints_path is not None:
            self.checkpoints_path = os.path.join(self.checkpoints_path, self.name)
        self.gif_path = os.path.join(self.checkpoints_path, "eval_gifs")
...
def eval(config: TrainConfig):
    ...
    print("---------------------------------------")
    print(f"Evaluating IQL, Env: {config.env}, Seed: {seed}")
    print("---------------------------------------")
    ...
    evaluations = []
    for t in range(config.eval_iter):
        # batch = replay_buffer.sample(config.batch_size)
        # batch = [b.to(config.device) for b in batch]
        # log_dict = trainer.train(batch)
        # wandb.log(log_dict, step=trainer.total_it)
